...
class TestFile extends StatefulWidget{
  ...
  @override
  State<StatefulWidget> createState() => _TestFileState();

}
class _TestFileState extends MountedState<TestFile> with SingleTickerProviderStateMixin{
  ...
  insertDeliveryFee() async{
    await DeliveryFeeDao().getAllDeliveryFees().then((value) async{
      if(value.length>0) {
        print("Checkinggggggg for save");
        await DeliveryFeeDao()
            .updateDeliveryFee(DeliveryFeeModel(1, activateDeliveryFee,
            double.parse(deliveryFeeController.text.split("${decimalSeparator}")[0].toString()),
            double.parse(deliveryFeeController.text.split("${decimalSeparator}")[0].toString()), displayNameController.text));
      }
      else{
        await DeliveryFeeDao()
            ...
            double.parse(deliveryFeeController.text.split(currency)[0].toString()), "Delivery Fee"));
      }
    });
  }
