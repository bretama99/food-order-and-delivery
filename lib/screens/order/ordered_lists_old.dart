...
class OrderedListOld extends StatefulWidget {
  //final String id;
  //final String isEatIn;
  //final String isTakeAway;
  final int tabIndex = 0;
  OrderedListOld( {Key? key}) : super(key: key);
  @override
  State<OrderedListOld> createState() => _OrderedListStateOld();
}

class _OrderedListStateOld extends MountedState<OrderedListOld> with SingleTickerProviderStateMixin{
 ...
 List<OrderModel> restaurantOrder=[];
  ...
  void initState(){
    ...
    FBroadcast.instance().register(ConstantBroadcastKeys.KEY_ORDER_SENT, (value, callback) async {
      print("Nowwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww vvvvvvvvvvvvvvvvvvvvv");
      await getOrderList();
      setState((){
        /*if(value.orderType==ConstantOrderType.ORDER_TYPE_RESTAURANT)
          restaurantOrder.add(value);
        if(value.orderType==ConstantOrderType.ORDER_TYPE_DELIVERY)
          deliveryOrder.add(value);*/
      ...
      case ConnectionState.done:
        if (snapshot.hasError) {
          ...
          return TabBarView(
            ...
            children: [
              SafeArea(
                  child: CommonFileOrderList(ConstantOrderType.ORDER_TYPE_RESTAURANT, Utility().getAttachedSortedOrderList(
                      snapshot.data!.where((element) => element.orderType == ConstantOrderType.ORDER_TYPE_RESTAURANT).toList(),
                      //orderNumber, 'restaurant', restaurantOrder, deliveryOrder),
              ...
              SafeArea(
                  child: CommonFileOrderList(ConstantOrderType.ORDER_TYPE_DELIVERY,Utility().getAttachedSortedOrderList(
                  snapshot.data!.where((element) => element.orderType == ConstantOrderType.ORDER_TYPE_DELIVERY).toList(),
                  //orderNumber, 'delivery', restaurantOrder, deliveryOrder),
