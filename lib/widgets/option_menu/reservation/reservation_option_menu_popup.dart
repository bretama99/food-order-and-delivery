...
class ReservationOptionMenuPopup extends StatefulWidget{
  Function onSelect;
  static _OrderOptionMenuesAction ACTIONS = _OrderOptionMenuesAction();
  ReservationOptionMenuPopup({required this.onSelect}){

  }
  @override
  State<StatefulWidget> createState() => _OptionMenuState();
}
class _OptionMenuState extends MountedState<ReservationOptionMenuPopup>{
  @override
                      ...
                      child: Column(
                        children: [
                          ...
                          OptionMenuTile(SvgPicture.asset(AppImages.editRedIcon, height: 30),Colors.white,"EDIT" ,(){
                            Navigator.pop(context);
                            widget.onSelect(ReservationOptionMenuPopup.ACTIONS.ACTION_EDIT);
                          }),
                          OptionMenuTile(SvgPicture.asset(AppImages.finishFlag, height: 30,color: Colors.white,),AppTheme.colorGreen,"ARRIVED" ,(){
                            Navigator.pop(context);
                            widget.onSelect(ReservationOptionMenuPopup.ACTIONS.ACTION_ARRIVED);
                          }),
                          OptionMenuTile(SvgPicture.asset(AppImages.deleteIcon, height: 30,color: Colors.white),AppTheme.colorRed,"DELETE", (){
                            Navigator.pop(context);
                            widget.onSelect(ReservationOptionMenuPopup.ACTIONS.ACTION_DELETE);
                          }),
                        ],
                      ),
                    ),
...
class _OrderOptionMenuesAction
{
  ...
  String ACTION_ARRIVED = "action_arrived`";
