...
class CompanyOptionMenuPopup extends StatefulWidget{
  Function onSelect;
  bool isShowAddressOption = false;
  static _OrderOptionMenuesAction ACTIONS = _OrderOptionMenuesAction();
  CompanyOptionMenuPopup({required this.onSelect,this.isShowAddressOption = false}){

  }
  @override
  State<StatefulWidget> createState() => _OptionMenuState();
}
class _OptionMenuState extends MountedState<CompanyOptionMenuPopup>{
  @override
                      ...
                      child: Column(
                        children: [
                          ...
                          OptionMenuTile(SvgPicture.asset(AppImages.editRedIcon, height: 30),Colors.white,"EDIT" ,(){
                            Navigator.pop(context);
                            widget.onSelect(CompanyOptionMenuPopup.ACTIONS.ACTION_EDIT);
                          }),
                          OptionMenuTile(SvgPicture.asset(AppImages.deleteIcon, height: 30,color: Colors.white),AppTheme.colorRed,"DELETE", (){
                            Navigator.pop(context);
                            widget.onSelect(CompanyOptionMenuPopup.ACTIONS.ACTION_DELETE);
                          }),
                        ],
                      ),
                    ),
