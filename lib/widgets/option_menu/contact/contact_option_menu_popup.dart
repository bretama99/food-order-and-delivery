...
class ContactOptionMenuPopup extends StatefulWidget{
  Function onSelect;
  bool isShowAddressOption = false;
  static _OrderOptionMenuesAction ACTIONS = _OrderOptionMenuesAction();
  ContactOptionMenuPopup({required this.onSelect,this.isShowAddressOption = false}){

  }
  @override
  State<StatefulWidget> createState() => _OptionMenuState();
}
class _OptionMenuState extends MountedState<ContactOptionMenuPopup>{
  @override
                      ...
                      child: Column(
                        children: [
                          ...
                          OptionMenuTile(SvgPicture.asset(AppImages.editRedIcon, height: 30),Colors.white,"EDIT" ,(){
                            Navigator.pop(context);
                            widget.onSelect(ContactOptionMenuPopup.ACTIONS.ACTION_EDIT);
                          }),
                          if(widget.isShowAddressOption)...[
                            OptionMenuTile(SvgPicture.asset(AppImages.clientAddressIcon, height: 30,color: Colors.white),Colors.black,"ADDRESSES", (){
                              Navigator.pop(context);
                              widget.onSelect(ContactOptionMenuPopup.ACTIONS.ACTION_ADDRESSES);
                            }),
                          ],
                          OptionMenuTile(SvgPicture.asset(AppImages.phoneClientIcon, height: 30,color: Colors.white,),AppTheme.colorGreen,"CALL" ,(){
                            Navigator.pop(context);
                            widget.onSelect(ContactOptionMenuPopup.ACTIONS.ACTION_CALL);
                          }),
                          OptionMenuTile(SvgPicture.asset(AppImages.deleteIcon, height: 30,color: Colors.white),AppTheme.colorRed,"DELETE", (){
                            Navigator.pop(context);
                            widget.onSelect(ContactOptionMenuPopup.ACTIONS.ACTION_DELETE);
                          }),
                        ],
                      ),
                    ),
...
class _OrderOptionMenuesAction
{
  ...
  String ACTION_ADDRESSES = "action_addresses";
