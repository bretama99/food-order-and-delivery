...
class AttributeOptionMenuPopup extends StatefulWidget{
  Function onSelect;
  static _OrderOptionMenuesAction ACTIONS = _OrderOptionMenuesAction();
  AttributeOptionMenuPopup({required this.onSelect}){

  }
  @override
  State<StatefulWidget> createState() => _OptionMenuState();
}
class _OptionMenuState extends MountedState<AttributeOptionMenuPopup>{
  @override
                      ...
                      child: Column(
                        children: [
                          ...
                          OptionMenuTile(SvgPicture.asset(AppImages.editRedIcon, height: 30),Colors.white,"EDIT" ,(){
                            Navigator.pop(context);
                            widget.onSelect(AttributeOptionMenuPopup.ACTIONS.ACTION_EDIT);
                          }),
                          OptionMenuTile(SvgPicture.asset(AppImages.deleteIcon, height: 30,color: Colors.white),AppTheme.colorRed,"DELETE", (){
                            Navigator.pop(context);
                            widget.onSelect(AttributeOptionMenuPopup.ACTIONS.ACTION_DELETE);
                          }),
                        ],
                      ),
                    ),
